...
def subscribejourney():  # used to wrap canary runs within a function
    ...
    # make custom Heap track API call
    ...
    heapTrackJson = {
        'app_id': '527942070',
        'identity': randomUserEmail,
        'event': 'Send Transactional Email',
        'properties': {'Transactional Email Subject': 'This Month with Play!',
                       'Transactional Email Variation': '' + emailVersion + ''},
        }
    json_data = json.dumps(heapTrackJson)
    emailConn.request('POST', '/api/track', json_data, headers)    
    ...
    if randomUTMSelector.endswith('1'):
        startingUrlWithUtmCodes = \
            'https://heap-sandbox-saas.vercel.app/main.html'
    elif randomUTMSelector.endswith('2'):
        startingUrlWithUtmCodes = \
            'https://heap-sandbox-saas.vercel.app/main.html'
    elif randomUTMSelector.endswith('3'):
        startingUrlWithUtmCodes = \
            'https://heap-sandbox-saas.vercel.app/main.html?utm_source=Facebook&utm_medium=display&utm_campaign=SanFranciscoGeo&utm_content=SaaSDemoNew'
    elif randomUTMSelector.endswith('4'):
        startingUrlWithUtmCodes = \
            'https://heap-sandbox-saas.vercel.app/main.html?utm_source=Twitter&utm_medium=display&utm_campaign=SanFranciscoGeo&utm_content=SaaSDemoNew'
    elif randomUTMSelector.endswith('5'):
        startingUrlWithUtmCodes = \
            'https://heap-sandbox-saas.vercel.app/main.html?utm_source=Blog&utm_medium=referral&utm_campaign=NewArticles&utm_content=BeTheBestSaaS'
    elif randomUTMSelector.endswith('6'):
        startingUrlWithUtmCodes = \
            'https://heap-sandbox-saas.vercel.app/main.html?utm_source=LinkedIn&utm_medium=display&utm_campaign=SaaSForExecutives&utm_content=ExecutiveContentSeries1'
    elif randomUTMSelector.endswith('7'):
        startingUrlWithUtmCodes = \
            'https://heap-sandbox-saas.vercel.app/main.html?utm_source=Google&utm_medium=cpc&utm_campaign=SponsortedContent&utm_content=NewProductFeatures'
    elif randomUTMSelector.endswith('8'):
        startingUrlWithUtmCodes = \
            'https://heap-sandbox-saas.vercel.app/main.html?utm_source=EmailList1&utm_medium=email&utm_campaign=UpgradePath&utm_content=TryNewFeatures'
    elif randomUTMSelector.endswith('9'):
        startingUrlWithUtmCodes = \
            'https://heap-sandbox-saas.vercel.app/main.html?utm_source=Affiliate&utm_medium=referral&utm_campaign=SaaSForExecutives&utm_content=ExecutiveContentSeries1'
    else:
        startingUrlWithUtmCodes = \
            'https://heap-sandbox-saas.vercel.app/main.html'
    ...
    # synthetics_logger.info('randomTrafficPatternSelection = '
    #                        + str(randomTrafficPatternSelection))  # use on AWS
    # synthetics_logger.info('randomPlanSelector value = '
    #                        + str(randomPlanSelector))  # use on AWS
    ...
    # synthetics_logger.info('selected userAgentString = '
    #                        + userAgentString)  # log UserAgentString
    # synthetics_logger.info('actual browser user agent string = '
    #                        + user_agent)  # log action browser user agent to see if setting agent string for browser worked
    ...
    if randomTrafficPatternSelection == 0:
        ...
        time.sleep(randomTimeSleepInterval)
        driver.find_element(By.ID, 'signUpEmailField'
                            ).send_keys(randomUserEmail)
        time.sleep(randomTimeSleepInterval)
        ...
        # make API call back to Heap for custom offline event
        ...
        heapTrackJson = {
            ...
            'identity': randomUserEmail,
            'event': 'Subscription',
            'properties': {'Backend Subscription Date': '' + todaysDate + '', 'Backend Subscription Level': '' + offlineSubscriptionDescription + ''},
        }
        json_data = json.dumps(heapTrackJson)
        subscribeConn.request('POST', '/api/track', json_data, headers)
